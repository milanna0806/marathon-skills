...
function ChatWidget() {
  ...
  const [items, setItems] = useState([
    { role: 'assistant', text: 'Привет! Я ИИ-консультант Marathon Skills. Спросите про регистрацию, дату, участников или статистику.' },
  ])
  ...
  const ask = async () => {
    ...
    try {
      ...
      setItems((prev) => [...prev, { role: 'assistant', text: data.answer || data.error || 'Не получилось ответить.' }])
    } catch {
      setItems((prev) => [...prev, { role: 'assistant', text: 'Связь с ИИ-агентом временно недоступна.' }])
    } finally {
      setSending(false)
    }
  }
  ...
  return (
    <div style={{position:'fixed',right:18,bottom:18,zIndex:300}}>
      {open && (
        <div style={{width:320,maxWidth:'calc(100vw - 36px)',height:420,maxHeight:'calc(100vh - 100px)',background:'#14142A',border:'1px solid #2D2D46',borderRadius:8,boxShadow:'0 12px 40px rgba(0,0,0,.45)',display:'flex',flexDirection:'column',overflow:'hidden',marginBottom:10}}>
          <div style={{height:44,background:'#1C1C33',borderBottom:'2px solid #E85D04',display:'flex',alignItems:'center',padding:'0 12px',gap:8}}>
            <span>🤖</span>
            <b style={{fontSize:12}}>ИИ-консультант</b>
            <button onClick={()=>setOpen(false)} style={{marginLeft:'auto',background:'transparent',border:'none',color:'#8C8CA5',fontSize:18,cursor:'pointer'}}>×</button>
          </div>
          <div style={{flex:1,overflowY:'auto',padding:10,display:'flex',flexDirection:'column',gap:8}}>
            {items.map((item, index) => (
              <div key={index} style={{alignSelf:item.role==='user'?'flex-end':'flex-start',maxWidth:'86%',background:item.role==='user'?'#E85D04':'#1E1E36',border:item.role==='user'?'none':'1px solid #2D2D46',borderRadius:7,padding:'8px 10px',fontSize:11,lineHeight:1.45,whiteSpace:'pre-wrap'}}>
                {item.text}
              </div>
            ))}
            {sending && <div style={{fontSize:10,color:'#8C8CA5'}}>ИИ думает...</div>}
          </div>
          <div style={{display:'flex',gap:6,padding:10,borderTop:'1px solid #2D2D46'}}>
            <input
              style={{...inputStyle,height:34,fontSize:11}}
              value={message}
              placeholder="Ваш вопрос..."
              onChange={(e)=>setMessage(e.target.value)}
              onKeyDown={(e)=>{ if (e.key === 'Enter') ask() }}
            />
            <button style={{...btnOrange,height:34,padding:'0 12px'}} onClick={ask} disabled={sending}>➤</button>
          </div>
        </div>
      )}
      <button onClick={()=>setOpen(v=>!v)} style={{...btnOrange,height:46,width:46,borderRadius:'50%',fontSize:20,justifyContent:'center',padding:0,boxShadow:'0 8px 24px rgba(0,0,0,.35)'}}>🤖</button>
    </div>
  )
}
...
// ── USERS TABLE ───────────────────────────────────────────────────
function UsersPage({ participants, isAdmin, onBack, onProfile, onEdit, onDelete, onBMI, onImported, loading }) {
  ...
  const importCsv = async (event) => {
    ...
    const data = await res.json()
    if (res.ok) {
      setImportMsg(`Загружено строк: ${data.imported}`)
      await onImported?.()
    } else {
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'100vh'}}>
      <nav style={{background:'#1C1C33',borderBottom:'2px solid #E85D04',height:58,display:'flex',alignItems:'center',padding:'0 14px',gap:10,flexShrink:0}}>
        ...
        {isAdmin && <>
          <input ref={importRef} type="file" accept=".csv,text/csv" style={{display:'none'}} onChange={importCsv}/>
          <button style={{...btnNav,marginLeft:'auto',height:32,fontSize:11}} onClick={exportCsv}>⬇ &nbsp;CSV</button>
          <button style={{...btnNav,height:32,fontSize:11}} onClick={()=>importRef.current?.click()}>⬆ &nbsp;Импорт CSV</button>
      ...
      <div style={{background:'#1E1E36',borderBottom:'2px solid #E85D04',padding:16}}>
        ...
        <div style={{display:'flex',flexWrap:'wrap',gap:10,alignItems:'center'}}>
          ...
          {importMsg && <span style={{fontSize:10,color:'#F4A33C'}}>{importMsg}</span>}
        </div>
      </div>
...
<div style={{height:140,borderRadius:10,overflow:'hidden',position:'relative',background:'linear-gradient(to right, rgba(232,93,4,.19), rgba(33,150,243,.06))',marginBottom:12}}>
  <div style={{display:'flex',gap:0,position:'absolute',top:'50%',left:'50%',transform:'translate(-50%,-60%)',justifyContent:'center'}}>
    {[0,1,2,3].map((i)=>(
      <div key={i} style={{borderRadius:10,width:90,height:70,overflow:'hidden',margin:'0 6px',border:'2px solid rgba(255,255,255,.2)',flexShrink:0,background:'linear-gradient(135deg,#2D2D46,#1E1E36)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:24}}>
        🏃
      </div>
    ))}
  </div>
